...
cl = LINETCR.LINE()
cl.login(token="EphhpS9BzY2lrw48F1Fa.apGNZMzba418M2CeQxD9/G.UWdYrLQBDw5BGUCu4K6joOATJCYIH9qqghmK2Ym6M5M=")
cl.loginResult()

ki1 = LINETCR.LINE() #ได้
ki1.login(token="Epj6J44MpLbXQuv0oyS0.jZPTeWFI8o4r3EGYcKY3Ka.bKzXHa18rlZkWTl4V3sqEx7ueb/Tv2IRdqeQ338N/XM=")
ki1.loginResult()

ki2 = LINETCR.LINE() #ได้
ki2.login(token="EpjVxnyJGU79YRoxFad2.n5XYTIS5UkLe7v+5uIuheG.S45FvnUK1EJ4PAe011z3rfx5/QavYSCgjnCjRFPD+HE=")
ki2.loginResult()

ki3 = LINETCR.LINE() #ได้
ki3.login(token="EpAp9osCS4K9LGTyPfPc.stjW/8zUEboHYYoVI1ovla.Bz83NmnkeW0yrpKKi+WabqWZeCw+7YYzEHCB6MODOVA=")
ki3.loginResult()

ki4 = LINETCR.LINE() #ได้
ki4.login(token="EpA9k5To76X6C8t8BAg1.r24R4PZfjzFugChKEJMPmq.0l+3N1F2/13PhHPXTtMv2dwi5at/XqcqHOvEmF/rY9A=")
ki4.loginResult()

ki5 = LINETCR.LINE() #ได้
ki5.login(token="EpoOaAJd0n3XvQYdsGI9.g8lrmrxeX7Rscw6kjxXpQq.QN/S+BPFWZ9uFGf6gb6dbOumohaVIbNnEsxt+KT4dik=")
ki5.loginResult()

ki6 = LINETCR.LINE() #ได้
ki6.login(token="EpX5ybocfLsEs5kYKAY5.Q8p1w/KIbJWNL53Bm1ltvq.maIqgCDEs1rfrjLFTHO7pjRAzsTs2LtT/bpN9Eu5whE=")
ki6.loginResult()

ki7 = LINETCR.LINE() #ได้
ki7.login(token="Ep2YQNOZpe5ou7J19FW8.5DBXg5omDCVRfRfxrLGz/a.8Bo8vKmtsSZp33QLI5U7JlDMMXF0913g4WkKdYDYLUA=")
ki7.loginResult()

ki8 = LINETCR.LINE() #ได้
ki8.login(token="Epsb75LHe4TbJ4UMkMR2.HDCYW7FjLWzQuFHaHTHxSG.QV8IaYEs9hwbSheLG2bXAKXQO/C/HHQrB5mjL8KyvC8=")
ki8.loginResult()

ki9 = LINETCR.LINE() #ได้
ki9.login(token="Epou5zAEJirJVrsGraPa.ki36fq0xIT+m2RdOv9EDsG.J5B9RU4M44KWT7KhDUqH4udHoD+C9o5HQEYGLDJel0c=")
ki9.loginResult()

ki10 = LINETCR.LINE() #ได้
ki10.login(token="EpZrpKmg3xuRmmqapm43.d3/yOmDTCR/imD36SHEf4W.6jitvI+z3WKjHNVvlgFOa1Wl8IxcUeQ2I2oD+tLCpJM=")
ki10.loginResult()
                ...
                if msg.from_ in admin:
                    ...
                    for target in targets:
                        try:
                            ...
                            pass
#----------------------------------------------------------------------------
            elif ("phet1n:" in msg.text):
              if msg.toType == 2:
                profile = ki1.getProfile()
                X = msg.text.replace("phet1n:","")
                profile.displayName = X
                ki1.updateProfile(profile)
                ki1.sendText(msg.to,"name " + X + " done")
              else:
                ki1.sendText(msg.to,"Failed")
            elif ("phet2n:" in msg.text):
              if msg.toType == 2:
                profile = ki1.getProfile()
                X = msg.text.replace("phet2n:","")
                profile.displayName = X
                ki2.updateProfile(profile)
                ki2.sendText(msg.to,"name " + X + " done")
              else:
                ki2.sendText(msg.to,"Failed")
            elif ("phet3n:" in msg.text):
              if msg.toType == 2:
                profile = ki3.getProfile()
                X = msg.text.replace("phet3n:","")
                profile.displayName = X
                ki3.updateProfile(profile)
                ki3.sendText(msg.to,"name " + X + " done")
              else:
                ki3.sendText(msg.to,"Failed")
            elif ("phet4n:" in msg.text):
              if msg.toType == 2:
                profile = ki4.getProfile()
                X = msg.text.replace("phet4n:","")
                profile.displayName = X
                ki4.updateProfile(profile)
                ki4.sendText(msg.to,"name " + X + " done")
              else:
                ki4.sendText(msg.to,"Failed")
            elif ("phet5n:" in msg.text):
              if msg.toType == 2:
                profile = ki5.getProfile()
                X = msg.text.replace("phet5n:","")
                profile.displayName = X
                ki5.updateProfile(profile)
                ki5.sendText(msg.to,"name " + X + " done")
              else:
                ki5.sendText(msg.to,"Failed")
            elif ("phet6n:" in msg.text):
              if msg.toType == 2:
                profile = ki6.getProfile()
                X = msg.text.replace("phet6n:","")
                profile.displayName = X
                ki6.updateProfile(profile)
                ki6.sendText(msg.to,"name " + X + " done")
              else:
                ki6.sendText(msg.to,"Failed")
            elif ("phet7n:" in msg.text):
              if msg.toType == 2:
                profile = ki7.getProfile()
                X = msg.text.replace("phet7n:","")
                profile.displayName = X
                ki7.updateProfile(profile)
                ki7.sendText(msg.to,"name " + X + " done")
              else:
                ki7.sendText(msg.to,"Failed")
            elif ("phet8n:" in msg.text):
              if msg.toType == 2:
                profile = ki8.getProfile()
                X = msg.text.replace("phet8n:","")
                profile.displayName = X
                ki8.updateProfile(profile)
                ki8.sendText(msg.to,"name " + X + " done")
              else:
                ki8.sendText(msg.to,"Failed")
            elif ("phet9n:" in msg.text):
              if msg.toType == 2:
                profile = ki1.getProfile()
                X = msg.text.replace("phet9n:","")
                profile.displayName = X
                ki9.updateProfile(profile)
                ki9.sendText(msg.to,"name " + X + " done")
              else:
                ki9.sendText(msg.to,"Failed")
            elif ("phet10n:" in msg.text):
              if msg.toType == 2:
                profile = ki1.getProfile()
                X = msg.text.replace("phet10n:","")
                profile.displayName = X
                ki10.updateProfile(profile)
                ki10.sendText(msg.to,"name " + X + " done")
              else:
                ki10.sendText(msg.to,"Failed")
...
set:addblacklist
บัญ���ีดำแบ็คลิสคนไม่ให้เข้ากลุ่มส���่ง���ล้�����ส่งคอลแทคคนที่เราจะแบ็คลิสลงในกลุ่ม
➖➖➖➖➖➖➖➖➖➖➖➖➖➖
...
#========================================
                ...
                else:md+="􀔃􀅉􏿿 Autorespon : off 􀜁􀄰􏿿\n"
...
#===========================================
                ...
                cl.sendText(msg.to, "ทำการบันทึกรูปภาพ..")
            elif msg.text in ["Papimage","pap","Pap"]:
